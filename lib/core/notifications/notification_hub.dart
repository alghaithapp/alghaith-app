...
class NotificationHub {
  ...
  List<RoleBannerData> taxiBannersFromDiff({
    ...
    for (final request in current) {
      ...
      if (prev == null &&
        ...
        _notify(
          title: 'طلب تكسي ${request.requestNumber}',
          body: '${request.pickupAddressAr} → ${request.dropoffAddressAr}',
          audience: 'driver',
          category: NotificationCategory.taxi,
          priority: NotificationPriority.urgent,
          eventKey: 'driver:${request.id}:new',
        );
        banners.add(RoleBannerData(
          title: 'رحلة جديدة!',
      ...
      if (prev == null || prev.statusKey == request.statusKey) continue;
      _notifyTaxiStatus(request);
      banners.add(RoleBannerData(
        title: 'تحديث رحلة ${request.requestNumber}',
        body: request.statusAr,
        colorHint: ColorHint.info,
      ));
    }
    return banners;
  ...
  void _notifyTaxiStatus(TaxiRequest request) {
    _notify(
      title: 'رحلة التكسي ${request.requestNumber}',
      body: request.statusAr,
      audience: 'customer',
      category: NotificationCategory.taxi,
      priority: NotificationPriority.urgent,
      eventKey: 'taxi:${request.id}:${request.statusKey}',
    );
    if (request.statusKey == 'accepted') {
      _notify(
        title: 'سائق قبل رحلتك',
        body: request.assignedDriverName?.trim().isNotEmpty == true
            ? request.assignedDriverName!
            : 'السائق في الطريق إليك',
        audience: 'customer',
        category: NotificationCategory.taxi,
        priority: NotificationPriority.urgent,
        eventKey: 'taxi:${request.id}:driver_accept',
      );
    }
  }
  ...
  void onTaxiRequestCreated(TaxiRequest request) {
    _notify(
      title: 'تم إرسال طلب التكسي',
      body: 'رقم ${request.requestNumber} — بانتظار سائق',
      audience: 'customer',
      category: NotificationCategory.taxi,
      priority: NotificationPriority.urgent,
      eventKey: 'customer:taxi:${request.id}:created',
    );
    _notify(
      title: 'طلب تكسي جديد',
      body: request.requestNumber,
      ...
      category: NotificationCategory.taxi,
      priority: NotificationPriority.urgent,
      eventKey: 'driver:taxi:${request.id}:created',
    );
  }
