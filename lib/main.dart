...
class _AlGhaithAppState extends State<AlGhaithApp> {
  ...
  void initState() {
    ...
    // عند الضغط على إشعار التكسي من شريط الإشعارات → فتح شاشة التكسي الجديدة
    PushNotificationInbox.onTaxiNotificationTapped = (requestId) {
      debugPrint('TaxiPushAction: فتح طلب $requestId من الإشعار');
      // TODO: فتح شاشة تفاصيل الطلب الجديدة من features/taxi/
    };
  }
...
// واجهة اختيار الشاشة المناسبة بناءً على حالة الحساب
    Widget getHome() {
      ...
      if (appProvider.userRole == 'merchant') {
        ...
        // إذا كان السائق معتمد لكن ناقصه مستندات (سنوية، بطاقة سكن، ...)
        final driverProfile = appProvider.driverProfile;
        if (!DriverProfileFields.hasRequiredUploads(driverProfile)) {
          return const ExitConfirmScope(child: DriverSetupScreen());
        }
...
// تم نقل وظيفة نافذة طلب التكسي إلى features/taxi/screens/
