...
class CustomerService extends ChangeNotifier {
  ...
  List<ListItem> get catalogItems =>
      List<ListItem>.unmodifiable(_catalogItems);
  MarketplaceStatsSnapshot? get marketplaceStats => _marketplaceStats;
  bool get marketplaceStatsLoading => _marketplaceStatsLoading;
  List<ActiveOrder> get orders =>
      List<ActiveOrder>.unmodifiable(_orders);
  ...
  String get merchantOpenTime => MerchantProfileFields.timeFromMap(
      _merchantStore, isOpen: true);
  String get merchantCloseTime => MerchantProfileFields.timeFromMap(
      _merchantStore, isOpen: false);
  ...
  bool get hasCompletedCustomerProfile {
    ...
    if (cp.isEmpty && (_authPhone == null || _authPhone!.trim().isEmpty)) return false;
  ...
  void applySkippedSetupFromRemote() {
    _skippedCustomerSetup = true;
    notifyListeners();
  }

  Map<String, dynamic> _buildRemoteState() => const {};
  ...
  List<ListItem> _buildLocalCatalogFallback() {
    ...
    return _items
        ...
        .map((item) => item.copyWith(
              merchantPhone: item.merchantPhone?.trim().isNotEmpty == true
                  ? item.merchantPhone
