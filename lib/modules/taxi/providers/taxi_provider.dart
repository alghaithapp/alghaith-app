...
class TaxiProvider extends ChangeNotifier {
  ...
  bool _isActiveCustomerTrip(TaxiRequest? request) {
    if (request == null) return false;
    return !request.isCompleted && !request.isCancelled;
  }

  Future<void> _applyCustomerActiveRequest({
    required TaxiRequest? previous,
    required TaxiRequest? remote,
  }) async {
    final active = _isActiveCustomerTrip(remote) ? remote : null;
    if (active != null) {
      _currentRequest = active;
      return;
    }
  ...
  Future<bool> completeTrip(String requestId) async {
    ...
    return updateStatus(trimmedId, 'completed');
  }
  ...
  Future<void> updateDriverTripLocation({
    ...
    try {
      ...
      if (updated != null && _currentRequest?.id == requestId) {
        final previous = _currentRequest;
        await _applyCustomerActiveRequest(previous: previous, remote: updated);
        notifyListeners();
      }
    } catch (_) {}
  }
  ...
  Future<void> loadActiveRequest() async {
    try {
      final previous = _currentRequest;
      final remote = await TaxiApiService.getActiveRequest();
      await _applyCustomerActiveRequest(previous: previous, remote: remote);
      notifyListeners();
    } catch (e) {
      _error = e.toString();
      notifyListeners();
