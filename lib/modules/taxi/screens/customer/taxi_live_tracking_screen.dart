...
class _TaxiLiveTrackingScreenState extends State<TaxiLiveTrackingScreen> {
  ...
  bool _exitAfterTripEnded = false;
  ...
  void _popIfTripEnded(TaxiRequest? request) {
    if (_exitAfterTripEnded || !mounted) return;
    final ended =
        request == null || request.isCompleted || request.isCancelled;
    if (!ended) return;
    _exitAfterTripEnded = true;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
  ...
  Future<void> _onMessageDriver(
    ...
    final id = context.read<TaxiProvider>().currentRequest?.id.trim() ?? '';
    final tripId = id.isNotEmpty ? id : (requestId?.trim() ?? '');
    if (tripId.isEmpty) {
      if (!mounted) return;
                          ...
                          children: [
                            ...
                            if (request != null &&
                                (request.canCustomerCancel ||
                                    request.canCustomerCompleteTrip)) ...[
                              const SizedBox(height: 14),
                              TaxiCustomerTripActions(
                                request: request,
                                onTripEnded: () {
