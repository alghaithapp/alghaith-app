                                ...
                                <article className="merchant-card account-card">
                                  ...
                                  <div className="merchant-actions">
                                    {needsApproval && isPending ? (
                                      <button className="soft-button success" disabled={approvalLoading || rejectLoading || suspendLoading} onClick={() => { handleApproveAccount(account).catch(() => undefined); }}>
                                    ...
                                    {needsApproval && (isPending || isRejected) ? (
                                      <button className="soft-button danger" disabled={approvalLoading || rejectLoading || suspendLoading} onClick={() => { openRejectConfirm({ phone: account.phone, displayName: account.displayName || account.phone, kind: account.kind }); }}>
                                        {rejectLoading ? <LoaderCircle className="spin" size={16} /> : <XCircle size={16} />}
                                        <span>رفض مع سبب</span>
                                      </button>
                                    ) : null}
                                    {isAdmin ? null : (
                                      <button className={account.isSuspended ? 'soft-button success' : 'soft-button danger'} disabled={suspendLoading || deleteLoading || approvalLoading || rejectLoading} onClick={() => { handleSuspendAccount(account).catch(() => undefined); }}>
                                        {suspendLoading ? <LoaderCircle className="spin" size={16} /> : <UserX size={16} />}
                                        <span>{account.isSuspended ? 'فك التعليق' : 'تعليق الحساب'}</span>
                                      </button>
                                    )}
                                    {isAdmin ? null : (
                                      <button className="soft-button danger" disabled={suspendLoading || deleteLoading || approvalLoading || rejectLoading} onClick={() => { openDeleteConfirm(account); }}>
                                        <Trash2 size={16} />
                                        <span>حذف الحساب</span>
                                      </button>
                                    )}
                                    {!isAdmin ? (
                                      <div className="role-change-wrapper">
                                        <select className="role-select" value={account.role || 'customer'} disabled={activeActionKey === `role:${account.phone}` || suspendLoading || deleteLoading}
                                          onChange={(event) => { handleChangeRole(account, event.target.value).catch(() => undefined); }}>
